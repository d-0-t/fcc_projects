...
function translate(text, dictionary, titles, timeFormat, locale) {
  ...
  if (notCapped) {
    for (let i = 0; i < notCapped.length; i++) {
      let notCappedReg = new RegExp(notCapped[i].toUpperCase(), "g");
      let cappedReg = new RegExp(notCapped[i].toUpperCase(), "g");
      translatedString = translatedString
      .replace(notCappedReg,cappedReg);
    }
  }
  ...
  let replaceRegex1 = new RegExp('<span class="highlight">', "g");
  let replaceRegex2 = new RegExp('</span>', "g");
  nonFormattedTranslation = translatedString
  .replace(replaceRegex1,'').replace(replaceRegex2,'');
